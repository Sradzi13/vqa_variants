...
def wups_similarity(sentence_1, sentence_2, info_content_norm):
    """
    Calculate the semantic similarity between two sentences. The last
    parameter is True or False depending on whether information content
    normalization is desired or not.
...
def compare_sentences(ref_sentences, produced_sentence):
    ...
    for sent in ref_sentences:
        scores.append((sent, produced_sentence, similarity(sent, produced_sentence, False), similarity(sent, produced_sentence, False)))
    return scores
