...
def get_best_synset_pair2(word_1, word_2):
    """
...
def most_similar_word(word, word_set):
    ...
    for ref_word in word_set:
        ...
        if sim > max_sim:
            max_sim = sim
            sim_word = ref_word
    #print('most_similar_word', sim_word, max_sim)
...
def wups(sentence_1, sentence_2):
    for w1 in sentence_1:
        w1_sys = wn.synset(w1)
        for w2 in sentence_2:
            w2_sys = wn.synset(w2)
            score = wups_word(w1_sys, w2_sys)

def wups_word(w1_sys, w2_sys):
    return w1_sys.wup_similarity(w2_sys)


def compare_sentences(ref_sentences, produced_sentence):
    scores = []
    for sent in ref_sentences:
        scores.append(similarity(sent, produced_sentence))
    return scores
